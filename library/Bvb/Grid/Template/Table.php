...
class Bvb_Grid_Template_Table {
    ...
    public function globalStart()
    {
        ...
        return "<table " . $this->getClass('table') . " border=\"0\" ".
               "cellspacing=\"0\" cellpadding=\"0\" id='gridTable'>" . PHP_EOL;
    }
    ...
    public function titlesLoop($title, $colspan)
    {
        ...
        return   "    <th ".$id."  " . $this->buildAttr('colspan', $colspan) . ">".Bvb_Grid_Translator::getInstance()->__($title)."</th>" . PHP_EOL;
       // return   "    <th ".$id." " . $this->buildAttr('colspna', $colspan) . ">$title</th>" . PHP_EOL;
    }
    ...
    public function loopStart($class, $style)
    {
        ...
        if($this->i%2 == 0)
        {
            ...
            $class = "odd";
        return "<tr " . $this->buildAttr('class', 'row') . " " . $this->buildAttr('style', $style) . ">";
    }
    ...
    public function loopLoop($value, $class, $style, $rowspan, $colspan)
    {
        ...
        return "        <td " . $this->buildAttr('class', $class) . " " . $this->buildAttr('style', $style) . " " .
                ...
                $this->buildAttr('colspan', $colspan) . " nowrap='nowrap'>$value</td>" . PHP_EOL;
    }
...
//                </td>" . PHP_EOL . "</tr></tfoot>" . PHP_EOL;
        $this->options['colspan'] += 1;
        return "    <tfoot><tr>" . PHP_EOL . "     <td " . $this->getClass('tableFooter') .
                " colspan=\"{$this->options['colspan']} \"><table cellspacing='0' cellpadding='0' border='0' width='100%'>
                <tr id='paging' " . $this->getClass('tableFooterPagination') . ">
                    <td class='showing' width='33%' align='left'> <em> ".Zend_Registry::get('recordsstart')." - ".Zend_Registry::get('recordsend')." of {$numberRecords} </em> </td>
                    <td width='33%' align='center' > {$perPage}</td>
                    <td class='rows_count' width='33%' align='right'> ".trim($this->result['pagination'][0],"| ")." </td>
                </table>
                </td>" . PHP_EOL . "</tr></tfoot>" . PHP_EOL;

                    //<td class='rows_count' width='33%' align='right'> ".trim($this->result['pagination'][0],"| ")."</td>
        /*return " <tfoot><tr id='paging' class='paging'>
                    <td width = '33%'   > &nbsp;</td>
                    <td width = '33%'> ". $perPage ." </td>
