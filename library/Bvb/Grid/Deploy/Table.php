...
class Bvb_Grid_Deploy_Table extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface {
                        ...
                        if ($orderField == $title['field']) {
                            ...
                            if (!array_key_exists('url', $title)) {
                                ...
                                if ($this->getAlwaysShowOrderArrows() === true
                                    ...
                                    //$link1 = "<a  href='$url1as'>{$images['desc']}</a>";
                                    //$link2 = "<a  href='$url2asd'>{$images['asc']}</a>";
                                    //$link1 = $images['desc'];
                                    //$link2 = $images['asc'];
                                    $link1 = "";
                                    $link2 = "";
    ...
    protected function _buildPagination()
    {
        ...
        if (count($this->_paginationInterval) > 0 && $this->getTotalRecords() > 0) {
             ...
             */
            $menuPerPage = '';
            foreach($this->_paginationInterval as $key => $val)
            {
                if($ppagina == $key) {
                    $arrmenuPerPage[] = "<span id='".$val."' class='perPage currentPerPage' style='cursor : pointer; padding: 5px;'> ".$val."</span>";
                } else {
                    $arrmenuPerPage[] = "<span id='". $val ."' class='perPage' style='cursor : pointer; padding: 5px;'> ". $val ."</span>";
                }
            }
            $menuPerPage = implode (" | ", $arrmenuPerPage);
                        // . ' ' . $this->__('items');
        } else {
            $menuPerPage = '';
        }
        ...
        Zend_Registry::set("recordsstart", ((($actual * $ppagina)- $ppagina) + 1));
        Zend_Registry::set("recordsend", ($actual * $ppagina));
       ...
       /*if ($actual != 1) {
            $pag = " <a id='gridFirstPage_{$ppagina}' onclick='getNextData({$ppagina})' href=\"$url/start/$ppagina\">" . $this->__('<<')
                 . "</a>&nbsp;&nbsp;<a id='grid_previousPage' onclick='getNextData({$ppagina})' href=\"$url/start{$this->getGridId()}/" . (($actual - 2) * $ppagina)
                 . "\">" . $this->__('>>') . "</a>&nbsp;&nbsp;" . $pag;
        }

        if ($actual != $npaginas) {
            $next = $actual * $ppagina;
            $prev = ($npaginas - 1) * $ppagina;

            $pag .= $actual ."&nbsp;&nbsp;<a id='gridNextPage_{$next}' onclick='getNextData({$next})' href=\"$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "\">"
                  . $this->__('>') . "</a>&nbsp;&nbsp;<a id='gridLastPage{$prev}' onclick='getNextData({$prev})' href=\"$url/start{$this->getGridId()}/"
                  . (($npaginas - 1) * $ppagina) . "\">" . $this->__('>>') . "</a>";
        }*/
        $pag = '';
        if ($actual != 1) {
            $prev = (($actual - 2) * $ppagina);
            $pag = " <a onclick='getNextData1(0)' href=\"javascript:void(0)\">" . $this->__('<<')
                 . "</a>&nbsp;<a onclick='getNextData1({$prev})' href=\"javascript:void(0)\">" . $this->__('<') . "</a>&nbsp;" . $pag;
        }
        ...
        if ($actual != $npaginas) {
            $next = $actual * $ppagina;
            $last = ($npaginas - 1) * $ppagina;

            $pag .= "&nbsp;<a onclick='getNextData1({$next})' href=\"javascript:void(0)\">"
                  . $this->__('>') . "</a>&nbsp;<a onclick='getNextData1({$last})' href=\"javascript:void(0)\">" . $this->__('>>') . "</a>";
        }
        ...
        if ($npaginas > 0 || count($this->_export) > 0) {
            //get actual record
            if ($actual <= 1) {
                $registoActual = 1;
    ...
    public function deploy()
    {
        ...
        if (($this->getParam('gridmod') == 'ajax' && $this->getInfo("ajax") !== false )
            ...
            $response->clearBody();
	    //$response->setBody("<tr id='{$gridId}'>" . implode($this->_renderDeploy)."</tr>" )
        ...
        //$grid = "<tr id='{$gridId}'>" . implode($this->_renderDeploy) . "</tr>";
        $grid = implode($this->_renderDeploy);
    ...
    private function _buildGridRender($deploy = true)
    {
        ...
        if (isset($this->_paginationInterval[0]) && $this->_paginationInterval[0] == 0)  {
            $bPagination = false;
        } else {
