(function(a) {
	a.fn.dashboard = function(m) {
			...
			a("." + b.columnClass)
					.sortable(
							{	
								...
								stop : function(o, p) { 
									var countWidget=1;
									$(".column .ui-widget").each(function(){
										var WidgetAttrid=this.id;
										var attrColumn=k.widgets[WidgetAttrid].column;
										if(typeof attrColumn=="undefined"){
											attrColumn="first";
										}
										setOrderofWidget(WidgetAttrid,countWidget,attrColumn);
										countWidget++;
									});
									setTimeout(function() {
		...
		k.loadWidgets = function(n) {
			...
			a("." + b.columnClass).each(
					function() {
						if (a(this).children().length == 0) {
							a(this).html(
									'<div class="emptycolumn">'
											+ b.emptyColumnHtml + "</div>")
						}
					});
			k.initialized = true			
		};
		...
		var l = a.extend({}, a.fn.dashboard.defaults.addWidgetSettings,
				m.addWidgetSettings);
		var d = a.extend({}, a.fn.dashboard.defaults.editLayoutSettings,
				m.editLayoutSettings);
		...
		function h(n) {
			...
			n.openContent = function() {
				...
				if (!n.loaded) {
					if (this.url != "" && this.url != null && typeof this.url != "undefined") {
						...
						n.element.find("." + b.widgetContentClass).load(
								this.url,
								function(p, o, q) {
									if (o == "error") {
				...
				if (k.initialized) {
					k.log("dashboardStateChange event thrown for widget "
							+ n.id, 2);
					k.element.trigger("dashboardStateChange", {
			...
			n.editDialog = function() {
				var editTitle = 'Edit '+n.title;
				$( "#editwidgetdialog" ).dialog({ 
					title: editTitle,
					resizable: true,
					modal: true,
					height:350,
					...
					beforeClose : function(event, ui){
						if($(this).data('id'))
			        	{
			        		if(($(this).data('id').saveclose) == 'saveclose')
			        		{
			        			$(this).data('id').saveclose = '';
			        			var id = $(this).data('id').data;
			        			var infoid = getWidgetByInfoID(id);
			        			var split_data = infoid.split("|");
			        			var unique_id = split_data[6]+'-'+split_data[0];
			        			var widgetID = split_data[6]+'-'+split_data[2]+'-'+id ;
			        			var jsondata = jQuery.parseJSON(split_data[4]);
			        			$("#"+widgetID).find('.widgettitle').text(jsondata.title);
			        		}
			        	}
						if($(this).data('graphid'))
			        	{
			        		if(($(this).data('graphid').saveclose) == 'saveclose')
			        		{
			        			$(this).data('graphid').saveclose = '';
			        			var id = $(this).data('graphid').data;
								var infoid = getWidgetByInfoID(id);
								var split_data = infoid.split("|");
								var unique_id = split_data[6]+'-'+split_data[0];
								var widgetID = split_data[6]+'-'+split_data[2]+'-'+id ;
						...
						$("#"+widgetID+" .widgetRefresh").trigger("click");
					}
				});	
				$.ajax({
					  type: 'POST',
			...
			n.closeContent = function() {
				...
				k
						.log("dashboardStateChange event thrown for widget "
								+ n.id, 2);
				k.element.trigger("dashboardStateChange", {
			...
			n.serialize = function() {
				k.log("entering serialize function", 1);
				var q = '{"title" : "' + n.title + '", "id" : "' + n.id
						+ '", "column" : "' + n.column + '","editurl" : "'
						+ n.editurl + '","open" : ' + n.open + ',"url" : "'
						+ n.url + '"';
				if (typeof n.metadata != "undefined") {
					q += ',"metadata":{';
					var s = n.metadata;
			...
			n.openFullscreen = function() {
				...
				//n.element.clone().appendTo(o);
				n.element.prependTo(o);
			};
		...
		a("#" + k.id + " .menutrigger").live(
				"click",
				function() {
					k.log("widgetOpenMenu event thrown for widget " + h.id, 2);
					var n = k.getWidget(a(this).closest("." + b.widgetClass)
							.attr("id"));
		...
		a("#" + k.id + " ." + b.widgetFullScreenClass)
				.live(
						"click",
						function(o) {
		...
		a("#" + k.id + " ." + b.widgetClass).live("widgetCloseMenu",
				function(n, p) {
					k.log("Closing menu " + a(this).attr("id"), 1);
					p.widget.closeMenu()
				});
		a("#" + k.id + " ." + b.widgetClass).live("widgetOpenMenu",
				function(n, p) {
					k.log("Opening menu " + a(this).attr("id"), 1);
					p.widget.openMenu()
				});
		a("#" + k.id + " ." + b.widgetClass).live("widgetDelete",
				function(n, p) {
		...
		a("#" + k.id + " .widgetRefresh").live("click",function(o) {
				var n = k.getWidget(a(this).closest("." + b.widgetClass).attr("id"));
				n.element.trigger("widgetRefresh", {
					widget : n
				})
		});		
		a("#" + k.id + " ." + b.widgetClass).live("widgetRefresh",
				function(n, p) { 
					p.widget.refreshContent()
				});		
		a("#" + k.id + " ." + b.widgetClass).live("widgetEdit",
				function(n, p) {
					p.widget.editDialog()
				});
		
		a("#" + k.id + " ." + b.widgetClass).live("widgetSetTitle",
				function(n, p) {
					p.widget.setTitle(p.title)
				});
		a("#" + k.id + " ." + b.widgetClass).live("widgetClose",
				function(n, p) {
		...
		a("#" + k.id + " ." + b.widgetClass).live(
				"widgetAddMetadataValue",
				function(n, p) {
					k.log("Changing metadata for widget " + a(this).attr("id")
							+ ", metadata name: " + p.name + ", value: "
							+ p.value, 1);
					p.widget.addMetadataValue(p.name, p.value)
				});
		a("#" + k.id + " ." + b.widgetTitleClass)
				.live(
						"click",
						function(o) {
							k.log("Click on the header detected for widget "
									+ a(this).attr("id"), 1);
							if (!a(this).hasClass("noclick")) {
								var n = k.getWidget(a(this).closest(
										"." + b.widgetClass).attr("id"));
								if (n.open) {
									k.log(
											"widgetClose event thrown for widget "
													+ n, 2);
									n.element.trigger("widgetClose", {
										widget : n
									})
								} else {
									k.log("widgetOpen event thrown for widget "
											+ n, 2);
		...
		a("#" + k.id + " ." + b.widgetHeaderClass).live("mouseover",
				function() {
		...
		k.element
				.live(
						...
						function() {
							if (typeof b.stateChangeUrl != "undefined"
									&& b.stateChangeUrl != null
									&& b.stateChangeUrl != "") {
								a.ajax({
											type : "POST",
			...
			a("." + d.selectLayoutClass)
					.bind(
							...
							function(q) {								
								...
								editedLayout(k.layout.id,o.vid,k.layout.vid);								
								...
								if (typeof b.layoutClass != "undefined") {
									...
									if (a("." + b.columnClass).length == 0) {
								 	...
								 	k.element.find(o.classname).each(
											function() {														
												a(this).children().prependTo(k.element.find("."+ b.columnClass+ ":first"));
									...
									k.element.find('.nonsortablecolumn').each(
										function() {
											a(this).children().prependTo(k.element.find("."+ b.columnClass+ ":first"));
											a(".emptycolumn").remove();		
											a("." + b.columnClass).each(function() {
		...
		k.element.live("addWidgetDialogSelectCategory",
				function(o, n) {
	...
	a.fn.dashboard.defaults = {
		...
		widgetRefreshClass : "widgetRefreshCustom",
		iconsClass : "icons",
		stateChangeUrl : "",
		addWidgetSettings : {
